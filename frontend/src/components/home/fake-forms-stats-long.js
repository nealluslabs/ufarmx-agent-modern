...
const columns =  !isMobile ? [
  ...
  { field: 'approvedDate', headerName: 'Date', width: 160,height:950, renderCell: (params) => {
    return <div style={{fontSize:"1rem",display:"flex",flexDirection:"column"}}>
      {params.row.updatedAt && new Date(params.row.updatedAt).toDateString() }
      
       <div style={{fontSize:"0.6rem",display:"flex",flexDirection:"column"}}>
      {params.row.updatedAt && new Date(params.row.updatedAt).toLocaleTimeString('en-US', {
     hour: 'numeric',
     minute: '2-digit',
     hour12: true
      }) }
  ...
  }, },
  {
    ...
    renderCell: (params) => {
      ...
      return <div style={{display:"flex",alignItems:"center",justifyContent:"flex-start",gap:"2.5rem",width:280}}>
       ...
       <span style={{display:"flex",alignItems:"flex-start",justifyContent:"flex-start",textAlign:"left",width:"40px",fontSize:"1rem",position:"relative",left:"-0rem"}}>{params.row.farmerName &&params.row.farmerName.length > 12 ?`${params.row.farmerName.slice(0,11)}...` :params.row.farmerName}</span>
...
"#0A9C36"
  ...
  { field: 'status', headerName: 'Status', width: 110,height:450, renderCell: (params) => {
    return <div style={{backgroundColor:params.row.status &&(params.row.status.toLowerCase() ==="approved" || params.row.status.toLowerCase()==="activated" &&params.row.status.toLowerCase())?"#B4DAC5":params.row.status==="Denied"?"#F9B8C5":"#E4E7EC",borderRadius:"20px",color:params.row.status==="Approved"?"#0B7D3E":params.row.status==="Denied"?"#D8173D":"black",padding:"1px",paddingLeft:"4px",paddingRight:"4px",width:"90px",textAlign:"center",border:"1px solid #DEDEDE",fontSize:"1rem"}}>{params.row.status}</div>;
  }},
